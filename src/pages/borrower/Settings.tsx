...
function ProfileTab() {
  ...
  const [incomeFrequency, setIncomeFrequency] = useState<string | null>(null)
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [{ data: profile }, { data: borrowerProfile }] = await Promise.all([
        supabase.from("profiles").select("name").eq("id", user.id).single(),
        supabase.from("borrower_profiles").select("*").eq("borrower_id", user.id).single(),
      ])
      if (!active) return
      setName(profile?.name ?? "")
      setPhone(borrowerProfile?.phone ?? null)
      setOccupation(borrowerProfile?.occupation ?? null)
      setIncomeFrequency(borrowerProfile?.income_frequency ?? null)
      setIncomeAmount(borrowerProfile?.income_amount ?? 0)
      setLoading(false)
    }
    load()
    return () => { active = false }
  ...
  const fields = [name, phone, email, occupation, incomeFrequency, incomeAmount > 0]
  ...
  return (
    <div>
      <div className="rounded-2xl p-6 mb-1 relative overflow-hidden" style={{ background: "linear-gradient(135deg, rgba(20,184,166,0.08) 0%, rgba(59,130,246,0.05) 45%, rgba(99,102,241,0.08) 100%)", border: "1px solid rgba(20,184,166,0.16)" }}>
        <div className="relative flex items-center gap-5">
          ...
          <div className="flex-1">
            ...
            <div className="text-xs mt-0.5" style={{ color: "var(--muted-foreground)" }}>Borrower</div>
          </div>
      ...
      <Group>
        ...
        <InfoRow iconKey="briefcase" label="Occupation" value={occupation || "Not set"} last />
      </Group>
      <SectionLabel>Financial Profile</SectionLabel>
      <Group>
        <InfoRow iconKey="dollar" label="Income Frequency" value={incomeFrequency ? (incomeFrequency === "daily" ? "Daily income" : "Monthly income") : "Not set"} />
        <InfoRow iconKey="dollar" label="Approximate Income" value={incomeAmount > 0 ? `\u20B9${incomeAmount.toLocaleString("en-IN")} / ${incomeFrequency === "daily" ? "day" : "month"}` : "Not set"} masked={incomeAmount > 0} last />
      </Group>
